...
class ConfigLoader:
    ...
    def validate_config(config: Dict[str, Any]):
        ...
        # Validate releases section if present
        if "releases" in config:
            ...
            for idx, release in enumerate(config["releases"]):
                ...
                for file_idx, file_config in enumerate(release["files"]):
...
class FleetManager:
    ...
    def load(self):
        ...
        self.releases = self.config.get("releases", [])
    ...
    def process(self):
        """Process all repositories and releases

        Returns:
            True if all repositories and releases processed successfully
        """
        if not self.config:
            self.load()
        ...
        if self.repositories:
            if self.max_workers > 1 and len(self.repositories) > 1:
                logger.info(
                    f"Using parallel processing with {self.max_workers} workers for repositories"
                )
                results = self.process_parallel()
            else:
                logger.info("Using sequential processing for repositories")
            ...
            repo_success = all(results)

        # Process releases
        release_success = self.process_releases()

        return repo_success and release_success
    ...
    def process_releases(self):
        """Process all release configurations

        Returns:
            True if all releases processed successfully
        """
        if not self.releases:
            return True

        logger.info(f"Processing {len(self.releases)} releases")

        asset_manager = ReleaseAssetManager(self.base_dir, self.dry_run)

        results = []
        for release_config in self.releases:
            result = asset_manager.process_release(release_config)
            results.append(result)

        success_count = sum(1 for r in results if r)
        logger.info(
            f"Processed {len(results)} releases: "
            f"{success_count} succeeded, {len(results) - success_count} failed"
        )
...
class ReleaseAssetManager:
    """Handles downloading and extraction of GitHub release assets"""

    def __init__(self, working_dir: str, dry_run: bool = False):
        """Initialize release asset manager
    ...
    @staticmethod
    def parse_release_url(release_url: str) -> Tuple[str, str]:
        """Parse GitHub release URL to extract owner and repo
    ...
    def download_asset(
        self, owner: str, repo: str, tag: str, asset_name: str, dest_path: str
    ) -> bool:
        ...
        if self.dry_run:
            logger.info(
                f"[DRY RUN] Would download {asset_name} from {owner}/{repo}:{tag} to {dest_path}"
            )
            return True
        ...
        try:
            logger.info(f"Downloading {asset_name} from {owner}/{repo}:{tag}")
    ...
    def extract_archive(self, file_path: str, extract_to: str) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"[DRY RUN] Would extract {file_path} to {extract_to}")
            return True
    ...
    def process_release(self, release_config: Dict[str, Any]) -> bool:
        """Process a single release configuration

        Args:
            release_config: Release configuration dictionary

        Returns:
            True if successful
        """
        try:
            url = release_config["url"]
            tag = release_config["tag"]
            files = release_config["files"]

            # Parse GitHub URL
            owner, repo = self.parse_release_url(url)

            logger.info(f"Processing release {owner}/{repo}:{tag}")

            all_success = True
            for file_config in files:
                asset_name = file_config["name"]
                dest = file_config.get("dest")
                extract = file_config.get("extract", True)
                ...
                # Download asset
                if not self.download_asset(owner, repo, tag, asset_name, dest_path):
                    all_success = False
                    continue

                # Extract if requested
                if extract:
                    extract_path = os.path.dirname(dest_path)
                    if not self.extract_archive(dest_path, extract_path):
                        all_success = False
