...
| `files`       | List of asset file objects to download                   | Yes      | -       |

Each file object in `files` supports:

| Property   | Description                                 | Required | Default |
|------------|---------------------------------------------|----------|---------|
| `name`     | Asset file name in the release              | Yes      | -       |
| `dest`     | Local destination path (relative/absolute)  | No       | working dir |
...
- `dest` を省略した場合は作業ディレクトリ直下に保存されます。
